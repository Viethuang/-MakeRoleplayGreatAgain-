	...
	if(characterPickTime[playerid] > 0) 
	{
		for (new i; i != MAX_CHARSELECT; i++)
		{
			if (playertextid == charSelectTextDraw[playerid][charSelectTextDrawID[playerid][i]])
			{
				...
				format(string, sizeof(string), "Ban da chon - {145F0B}%s{FFFFFF}", characterLister[playerid][i]);
...
{	
	if(!cache_num_rows())
	{
		...
		for(new i=0; i<maxname; i++)
		{
			if(pname[i] == '_')
			{
				SendServerMessage(playerid, "Ban chua nhap username cua minh, vui long ra ngoai va nhap username cua ban de dang nhap.");
				return Dialog_Show(playerid, DIALOG_SET_USERNAME, DIALOG_STYLE_INPUT, "Thay doi Username ma ban muon", "Nhap username de thay doi", ">", "Thoat");
			}
		}
	...
	{
		SendServerMessage(playerid, "|_________________IP BANNED_________________|"); 
		SendServerMessage(playerid, "IP \"%s\" da bi cam khoi may chu", ReturnIP(playerid));
		SendServerMessage(playerid, "|___________________________________________|"); 
...
public OnplayerCache(playerid)
{
	...
	if(C_DUTY)
	{
		...
		{
			SendFactionMessageEx(playerid, 0xFF8282FF, "HQ: %s %s da thoat khoi may chu (Crashed)", ReturnFactionRank(playerid), ReturnName(playerid, 0));
		}
	}
...
public LoadCharacter(playerid)
{
	...
	{
		SendClientMessage(playerid, COLOR_LIGHTRED, "ERROR: {FFFFFF}Nhan vat cua ban chua duoc xac nhan. Vui long hoan thanh dang ky cua ban truoc khi tham gia tro choi.");
		SendClientMessage(playerid, COLOR_LIGHTRED, "ERROR: {FFFFFF}Neu ban da hoan thanh dang ky cho minh, vui long den xac nhan don dang ky cua ban tai Discord.");
		SendClientMessageEx(playerid, COLOR_YELLOW, "INFO: {FFFFFF}Ten UCP: %s Character code: %d Code UCP: %d", e_pAccountData[playerid][mAccName], PlayerInfo[playerid][pDBID], e_pAccountData[playerid][mDBID]);
	...
	foreach(new i : Player)
	{
		...
		SendAdminMessageEx(COLOR_LIGHTRED, 1, "Cung mot UCP dang nhap hai nhan vat cung mot luc (%d) %s va (%d) %s",playerid, ReturnName(playerid,0), i, ReturnName(i,0));
	}
	...
	if (!PlayerInfo[playerid][pTimeout]) {
		format(string, sizeof(string), "~w~Chao mung~n~~y~ %s", ReturnPlayerName(playerid));
		GameTextForPlayer(playerid, string, 1000, 1);
	}

	if (PlayerInfo[playerid][pAdmin])
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "SERVER: Ban da dang nhap voi tu cach la Admin %i", PlayerInfo[playerid][pAdmin]);
	}
	...
	{
		SendClientMessage(playerid, -1, "Ban dang nhap bang thiet bi Android");
	}
	else SendClientMessage(playerid, -1, "Ban dang nhap bang thiet bi PC/Laptop");
	...
	new str[120];
    format(str, sizeof(str), "[%s] %s : da tham gia may chu", ReturnDate(),ReturnName(playerid,0));
...
public CheckBanAccount(playerid)
{
	...
	if(rows)
	{
		SendClientMessage(playerid, COLOR_LIGHTRED, "Nhan vat nay cua ban da bi cam. Vui long lien he voi Admin de biet them.");
		SendAdminMessageEx(COLOR_LIGHTRED, 1, "Tai khoan %s dang co gang tham gia may chu.",e_pAccountData[playerid][mDBID]);
...
CreateNewCharacter(playerid) {
		...
		SendClientMessage(playerid, -1, "Huong dan tao nhan vat.");
		SendClientMessage(playerid, -1, "Vui long nhap Ho_Ten cho nhan vat cua ban - Vi du: {145F0B}Douglas_Hodge{FFFFFF}.");
		SendClientMessage(playerid, -1, "Ten nhan vat cua ban khong co so va ky tu dac biet.");

		characterPickTime[playerid] = 0;

		Dialog_Show(playerid, DIALOG_CREATE_CHARACTER, DIALOG_STYLE_INPUT, "Name Character", "Nhap ten vao ben duoi:", "Xac nhan", "Huy");
		...
		SendClientMessage(playerid, COLOR_YELLOWEX, "Tai khoan cua ban khong duoc tim thay trong du lieu");
		SendClientMessage(playerid, COLOR_YELLOWEX, "Vui long dang ky tai khoan trong Discord");
		KickEx(playerid);
...
Dialog:DIALOG_CREATE_CHARACTER(playerid, response, listitem, inputtext[])
{
	...
	if (IsValidRoleplayName(inputtext)) {
		...
		Dialog_Show(playerid, DIALOG_CREATE_CHARACTER, DIALOG_STYLE_INPUT, "Name Character", "Ten nhan vat khong hop le !!\n\nNhap ten nhan vat lai:", "Xac nhan", "Huy");
	}
	return 1;
}
...
ptask CharacterTimer[1000](playerid) {
    if(characterPickTime[playerid] > 0)
	{
		...
		{
			SendClientMessage(playerid, COLOR_LIGHTRED, "Ban da bi kick vi khong chon nhan vat.");
			KickEx(playerid);
		}
	}
    return 1;
...
public Query_SelectCharacter(playerid)
{
	if (!cache_num_rows())
	{
		SendClientMessage(playerid, -1, "[ERROR]: Da xay ra loi khi xu ly nhan vat cua ban.");
		ShowCharacterSelection(playerid);
		return 1;
	}
...
stock ResetPlayerCharacter(playerid)
{
	...
	SendClientMessage(playerid, COLOR_GREY, "Da xoa nhan vat thanh cong.");
	return 1;
}
