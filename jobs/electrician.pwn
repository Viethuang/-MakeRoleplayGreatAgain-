...
hook OP_PickUpDynamicPickup(playerid, STREAMER_TAG_PICKUP:pickupid)
{
    ...
    {
        SendClientMessage(playerid, -1, "Su dung /takejob de nhan cong viec.");
        return 1;
    }
    return 1;
}
...
hook OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    if(!ispassenger)
    {
        for(new i = 0; i < sizeof electrician_vehicles; i++) if(vehicleid == electrician_vehicles[i])
		{
			...
			{
				SendErrorMessage(playerid, "Ban phai o trong xe sua dien.");
...
CMD:getstair(playerid, params[])
{
    if(!IsPlayerInAnyVehicle(playerid) && GetNearestVehicle(playerid) != INVALID_VEHICLE_ID)
    {
        ...
        if(modelid != 552)
            return SendErrorMessage(playerid, "Phuong tien nay khong phai phuong tien cua cong viec.");
            
        if(VehicleInfo[vehicleid][eVehicleLocked])
            return SendServerMessage(playerid, "Xe da khoa.");

        if(!IsPlayerInRangeOfPoint(playerid, 2.5, x, y, z))
            return SendErrorMessage(playerid, "Ban khong dung gan diem.");
    ...
    else SendErrorMessage(playerid, "Ban khong co nhan cong viec nay nen khong the su dung lenh.");
    return 1;
}
...
{   
    if(!FixElePos[playerid])
        return SendErrorMessage(playerid, "Da den vi tri hay su dung /startele de bat dau sua.");
    ...
    {
        SendErrorMessage(playerid, "¤Ø³äÁèä´éÍÂÙèã¹¨Ø´«èÍÁàÊÒä¿¿éÒ·ÕèªÓÃØ´ ä»µÒÁ¨Ø´·ÕèàÃÒÁÒÃìÒ¡äÇéãËé¤Ø³");
    ...
    if(!GetPVarInt(playerid, "GetStair"))
        return SendErrorMessage(playerid, "¤Ø³ÂÑ§äÁèä´é¹ÓºÃÃä´ÍÍ¡ÁÒ¨Ò¡·éÒÂÃ¶");
...
{
    if(PlayerInfo[playerid][pJob] != JOB_ELECTRICIAN)
        return SendErrorMessage(playerid, "¤Ø³äÁèãªèÍÒªÕ¾ªèÒ§«èÍÁä¿");

    if(GetVehicleModel(GetPlayerVehicleID(playerid)) != 552)
        return SendErrorMessage(playerid, "¤Ø³äÁèä´éÍÂÙèº¹ÂÒ¹¾ÒË¹ÐÊÓËÃÑº¡ÒÃ«èÍÁä¿¿éÒ");

    if(FixElePos[playerid])
    {
        SendErrorMessage(playerid, "¤Ø³ÁÕ¨Ø´ä¿·ÕèÂÑ§äÁèä´é«èÍÁ");
        SendClientMessage(playerid, -1, "ä»µÒÁ¨Ø´áÅéÇ¾Ô¾Áì /fixele à¾×èÍ«èÍÁàÊÒä¿¿éÒ·ÕèªÓÃØ´àÊÕÂËÒÂ");
        SetPlayerCheckpoint(playerid, GoFixElePlace[FixElePos[playerid]][0], GoFixElePlace[FixElePos[playerid]][1], GoFixElePlace[FixElePos[playerid]][2], 2);
        return 1;
    }
    ...
    if(gettime() - timestart < 30)
        return SendErrorMessage(playerid, "¡ÃØ³ÒÃÍ 2 ¹Ò·Õ¡èÍ¹·Õè¨Ðä»«èÍÁä¿¿éÒ");

    new randdom = Random(1, 50);
    FixElePos[playerid] = randdom;
    SendClientMessage(playerid, -1, "ä»µÒÁ¨Ø´·ÕèàÃÒä´éÁÒÃìÒ¡äÇéãËé¤Ø³áÅéÇ¾Ô¾Áì /fixele à¾×èÍ«èÍÁàÊÒä¿¿éÒ·ÕèªÓÃØ´àÊÕÂËÒÂ");
    SetPlayerCheckpoint(playerid, GoFixElePlace[FixElePos[playerid]][0], GoFixElePlace[FixElePos[playerid]][1], GoFixElePlace[FixElePos[playerid]][2], 2);
    return 1;
}
...
public OnRepiarEle(playerid, Float:x, Float:y, Float:z)
{
    ...
    SendClientMessageEx(playerid, COLOR_DARKGREEN, "¤Ø³ä´é·Ó¡ÒÃ«èÍÁàÊÒä¿¿éÒ·ÕèªÓÃØ´ÊÓàÃç¨àÊÃç¨ÊÔé¹áÅéÇ ä´éà§Ô¹ÁÒ¨Ó¹Ç¹ $%s", MoneyFormat(money));
    SendClientMessageEx(playerid, COLOR_DEPT, "àÊÕÂÀÒÉÕÃÒÂä´é ÃéÍÂÅÐ 0.07 ($%s)",MoneyFormat(floatround(money * 0.07, floatround_round)));
...
stock GetStair(playerid)
{
    ...
    SendNearbyMessage(playerid, 3.0, COLOR_EMOTE, "> %s ËÂÔººÃÃä´ÍÍ¡ÁÒ¨Ò¡·éÒÂÃ¶ áÅÐ¶×ÍäÇé´éÇÂÁ×Í·Ñé§ÊÍ§¢éÒ§",ReturnName(playerid,0));
    return 1;
}
...
stock PlaceStair(playerid)
{
    ...
    SendNearbyMessage(playerid, 3.0, COLOR_EMOTE, "> %s ÇÒ§ºÃÃä´à¡çºäÇé·Õè·éÒÂÃ¶",ReturnName(playerid,0));
    return 1;
}
