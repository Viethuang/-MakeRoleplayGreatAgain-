...
hook OP_PickUpDynamicPickup(playerid, pickupid)
{
	if(pickupid == FarmerPickup || pickupid == FarmerPickup2) {
		GameTextForPlayer(playerid, "~w~Nhap /farmerjob de nhan cong viec ~n~farmer", 5000, 3);
		return 1;
	}
    return 1;
}
...
CMD:farmerjob(playerid)
{
    if(PlayerInfo[playerid][pLevel] > 3)
		return SendClientMessage(playerid, COLOR_LIGHTRED, "Cong viec nay chi danh cho cap do 1-3!");
        
	if(PlayerInfo[playerid][pFaction])
	    return SendClientMessage(playerid, COLOR_WHITE, "Xin loi, nghe nghiep nay chi danh cho dan thuong.");
	...
	if (IsPlayerInRangeOfPoint(playerid, 2.5, -382.5893, -1426.3422, 26.2217) || IsPlayerInRangeOfPoint(playerid, 2.5, -29.0948, 62.1961, 3.1172)) {
	    ...
	    if(PlayerInfo[playerid][pJob] != 0)
            return SendClientMessage(playerid, COLOR_LIGHTRED, "Su dung /leavejob de thoat cong viec.");

        PlayerInfo[playerid][pJob] = JOB_FARMER;
        GameTextForPlayer(playerid, "~r~Chuc mung,~n~~w~Ban da nhan cong viec ~y~Farmer.~n~~w~/jobhelp.", 8000, 1);
        SendClientMessage(playerid, COLOR_GRAD1, "Bay gio ban la mot farmer.");
		return 1;
	}
	else return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong dung gan diem xin viec.");
}

CMD:harvest(playerid)
{
	if(PlayerInfo[playerid][pJob] != JOB_FARMER)
	    return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong co cong viec Farmer!");

	new vehicleid = GetPlayerVehicleID(playerid);

	if (GetVehicleModel(vehicleid) != 532)
		return SendClientMessage(playerid, COLOR_LIGHTRED, "Ban phai o tren xe 'Combine Harvester'.");

	if (GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
	 	return SendClientMessage(playerid, COLOR_LIGHTRED, "Ban khong ngoi ghe lai cua xe.");

	if (IsPlayerInRangeOfPoint(playerid, 100.0, -377.8374,-1433.8853,25.7266)) {

		if(far_start{playerid})
            return SendClientMessage(playerid, COLOR_GRAD1, "Ban da bat dau thu hoach.");
		...
		SendClientMessage(playerid, COLOR_WHITE, "Ban da bat dau thu hoach mua mang.");
		...
		if(far_start[playerid]) return SendClientMessage(playerid, COLOR_GRAD1, "Ban da bat dau thu hoach.");
	...
	else return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong o trang trai.");
}
...
CMD:stopharvest(playerid)
{
	if(far_start{playerid})
	{
		...
		return SendClientMessage(playerid, COLOR_WHITE, "Ban da ngung thu hoach.");
	}
	else return SendClientMessage(playerid, COLOR_GRAD1, "	Ban chua bat dau thu hoach.");
}
            ...
            case 1: { // ส่งเก็บเกี่ยว
                ...
                if(far_veh[playerid] == vehicleid) {
                    if (IsPlayerInRangeOfPoint(playerid, 5.0, -53.5525,70.3079,4.0933) || IsPlayerInRangeOfPoint(playerid, 5.0, -377.8374,-1433.8853,25.7266)) {
                        ...
                        format(farmString, sizeof(farmString), "~w~Ban da co mot it lua mi va co the ban no voi gia~n~~g~%d$", randmoney);
